...
\footnotetext[5]{\Cree does not test if access to the server is blocked by a firewall, nor if the two required ports are open.}
...
\footnotetext[6]{\emph{jQuery} is used on 64\% of monitored websites. There is no statistics on the \emph{Bootstrap} CSS library, but its complementary JavaScript library is used on 8\% of websites. Source: \url{http://w3techs.com/technologies/overview/javascript_library/all}. While \emph{Node.js} is used as web-server on 0.1\% of monitored websites. Source: \url{http://w3techs.com/technologies/overview/web_server/all}.}
...
\footnotetext[7]{ie. when the subject uses a different tab in the web browser, when the browser is minimised or fully obscured by another application, or when the device enters its power saving mode (lock screen on smartphones). Source: \url{http://www.w3.org/TR/page-visibility/}}
